...
{
	/// <summary>
	/// Obtained from http://10rem.net/blog/2011/02/08/capturing-screen-images-in-wpf-using-gdi-win32-and-a-little-wpf-interop-help
	/// </summary>
    class ScreenCaptureHelper
    {
        ...
        private static Int32Rect GetWindowActualRect(IntPtr hWnd)
        {
			...
			// Commented out on 12/26/2013 4:58 PM
            //// sooo, yeah.
            ...
            //Originally commented: User32.ClientToScreen(hWnd, ref topLeftPoint);

			// I commented on 12/26/2013 4:58 PM
			...
			Win32Point topLeftPoint = new Win32Point(windowRect.Left, windowRect.Top);

			Int32Rect actualRect = new Int32Rect(
				topLeftPoint.X + sideBorder, // we remove sideBorder as we don't need it
				topLeftPoint.Y + 50, // 20 cause we remove the non-client window area
				windowRect.Width - sideBorder*2, //  + sideBorder * 2 + hackToAccountForShadow, -- we remove everything else
				windowRect.Height - 100 //   + sideBorder * 2 + hackToAccountForShadow -- we remove everything else
				);
