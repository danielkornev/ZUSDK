...
namespace System
{
	/// <summary>Provides extension methods for trigonometric, logarithmic, geometric, and other common mathematical functions.</summary>
	public static partial class MathExtensions
	{
		...
		public static double Distance(this Vector p1, Vector p2)
		{
			return Math.Sqrt((p2.X - p1.X) * (p2.X - p1.X) +
				(p2.Y - p1.Y) * (p2.Y - p1.Y));
		}

		public static double Distance(this Point p, Vector v)
		{
			return Math.Sqrt((v.X - p.X) * (v.X - p.X) +
				(v.Y - p.Y) * (v.Y - p.Y));
		}

		public static void Scale(this Point p, double scaleFactor)
		{
			p.X *= scaleFactor;
			p.Y *= scaleFactor;
		}
		...
		#region Speciial Operations
		public static bool TryGetPointWithMinimalY(this List<Point> points, out Point rPoint)
		{
			bool result = false;
			...
			else
			{
				...
				rPoint = points.FirstOrDefault(p => p.Y == yMin);

				result = true;
			}

			return result;
		} // TryGetPointWithMinimalY
