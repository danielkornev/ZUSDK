...
namespace System.Threading.Tasks.Schedulers
{
	...
	public sealed class StackedTaskScheduler : TaskScheduler, IDisposable
	{
		...
		private int _delegatesStackedOrRunning = 0;
		...
		/// <param name="taskWasPreviouslyStacked">Whether the task was previously queued.</param>
		...
		{
			// Create the queue
			var createdQueue = new StackedTaskSchedulerQueue(priority, this);
		...
		/// <summary>Provides a scheduling queue associatd with a StackedTaskScheduler.</summary>
		...
		private sealed class StackedTaskSchedulerQueue : TaskScheduler, IDisposable
		{
			...
			private sealed class StackedTaskSchedulerQueueDebugView
			{
				...
				/// <summary>Gets the StackedTaskScheduler with which this queue is associated.</summary>
				public StackedTaskScheduler AssociatedScheduler { get { return _queue._pool; } }
			}
			...
			protected override void QueueTask(Task task)
			{
				...
				lock (_pool._queueGroups) _workItems.Push(task);
			...
			public void Dispose()
			{
				if (!_disposed)
				{
					lock (_pool._queueGroups)
					{
						...
						// we still mark it as disposed, and the associated StackedTaskScheduler
